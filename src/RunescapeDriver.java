...
public class RunescapeDriver {
  ...
  private int[] getOffset() {
    if( offsetIndex < X_OFFSET.length ) {
      return new int[] { X_OFFSET[offsetIndex], Y_OFFSET[offsetIndex] };
    }
    else {
      return new int[] { customOffset.x, customOffset.y };
    }
  }
  ...
  private String[] offsetNames = { "-960", "-1920", "Custom" };
  ...
  public void setOffsetIndex(int index) {
    ...
    if( offsetIndex >= X_OFFSET.length ) {
      if( customOffset != null ) {
        if( mainFrame != null ) {
          mainFrame.setLocation(customOffset.x, customOffset.y);
        }
      }
    }
    else {
  ...
  public void loadSettings() {
    ...
    try {
      ...
      if( offsetIndex == offsetNames.length-1) {
        line = bufferedReader.readLine();
        if( line != null ) {
          StringTokenizer st = new StringTokenizer(line);
          int x = Integer.parseInt(st.nextToken());
          int y = Integer.parseInt(st.nextToken());
          customOffset = new Point(x, y);
        }
      }
  ...
  public void saveSettings() {
    ...
    try {
      ...
      if( offsetIndex == offsetNames.length-1 ) {
        bufferedWriter.newLine();
        bufferedWriter.write(mainFrame.getX() + "\t" + mainFrame.getY());
      }
  ...
  private void loadItems() {
    ...
    for( int i = 0; i < itemFiles.length; i++ ) {
      ...
      try {
        ...
        System.err.println("Loaded " + i + " " + itemNames[i]);
      } catch (IOException e) {
...
//    dropButton.setPreferredSize(new Dimension(100, 30));
    ...
    alchButton.setFocusable(false);
    alchButton.addActionListener(new ActionListener() {
      @Override
      public void actionPerformed(ActionEvent e) {
        stopAll();
        alch1();
...
//      }
  ...
  private void sleepTrueTime(long time) throws InterruptedException {
    Thread.sleep(time);
  }
...
//          sleep(13000);
  ...
  public class DisplayDialog extends JDialog {
    private static final int FRAMERATE = 300;
    private BufferedImage displayImage;
    private Semaphore timeToClick;
    ...
    public void updateImage(BufferedImage image, int time) {
      timeToClick.drainPermits();
      displayImage = image;
      timeToClick.release(time/FRAMERATE + 1);
      repaint();
    }
  }
  public void alch12() {
    // iron ore at al kharid
    startTime = System.currentTimeMillis();
    Thread thread = new Thread(new Runnable() {
      ...
      public void run() {
        ...
        try {
          ...
          mainFrame.repaint();
          sleep(5000);
          int items = itemsToDrop;
          while( items > 0 ) {
            robot.mousePress(InputEvent.BUTTON1_MASK);
            sleep((int)(getRandomGaussian(5)*50 + 50));
            robot.mouseRelease(InputEvent.BUTTON1_MASK);
            sleep((int)(getRandomGaussian(5)*50 + 60));
            String natureRuneItemName = "nature_rune3";
            boolean hasNatureRune = false;
            if( itemThere(prevNatureX, prevNatureY)  == itemNameMap.get(natureRuneItemName) ) {
              hasNatureRune = true;
            }
            if( !hasNatureRune || !hasCoin ) {
              System.err.println("searching for nature rune " + hasNatureRune + ", coins " + hasCoin);
            }
            for( int y = 0; y < 7 && (!hasNatureRune || !hasCoin ); y++ ) {
              for( int x = 0; x < 4 && (!hasNatureRune || !hasCoin ); x++ ) {
                int item = itemThere(x, y);
                if( item == itemNameMap.get(natureRuneItemName) ) {
                  hasNatureRune = true;
                  prevNatureX = x;
                  prevNatureY = y;
                }
                if( item == itemNameMap.get("coins2") ) {
                  hasCoin = true;
                  prevCoinX = x;
                  prevCoinY = y;
                }
            ...
            BufferedImage natureRuneImage = getItemImage(prevNatureX, prevNatureY);
            BufferedImage alchingItemImage = getItemImage(3, 3);
            BufferedImage coinImage = getItemImage(prevCoinX, prevCoinY);
            BufferedImage fullImage = new BufferedImage(natureRuneImage.getWidth() + alchingItemImage.getWidth() + coinImage.getWidth(), alchingItemImage.getHeight(), BufferedImage.TYPE_3BYTE_BGR);
            Graphics g = fullImage.getGraphics();
            g.drawImage(natureRuneImage, 0, 0, null);
            g.drawImage(alchingItemImage, natureRuneImage.getWidth(), 0, null);
            g.drawImage(coinImage, natureRuneImage.getWidth() + alchingItemImage.getWidth(), 0, null);
            g.dispose();
            int sleepTime = (int)(getRandomGaussian(2)*3500 + 2000);
            display.updateImage(fullImage, sleepTime);
            
            robot.mousePress(InputEvent.BUTTON1_MASK);
            sleep((int)(getRandomGaussian(10)*50 + 50));
            robot.mouseRelease(InputEvent.BUTTON1_MASK);
            if( !isItemThere(3, 3) || !hasNatureRune) {
              System.err.println("Ending alch because missing target or missing nature rune");
              break;
            }
            items--;
            
//            if( items == 100 ) {
//              MusicPlayer.playNote(38, 0);
//            }
//            if( items == 80 ) {
//              MusicPlayer.playNote(42, 0);
//            }
//            if( items == 60 ) {
//              MusicPlayer.playNote(45, 0);
//            }
//            if( items == 40 ) {
//              MusicPlayer.playNote(35, 0);
//            }
//            if( items == 20 ) {
//              MusicPlayer.playNote(40, 0);
//            }
//            if( items == 1 ) {
//              MusicPlayer.playNote(44, 0);
//            }
            if( items != 0 ) {
      ...
      public void run() {
        ...
        try {
          ...
          while( items > 0 ) {
            ...
            BufferedImage alchingItemImage = getItemImage(3, 3);
            BufferedImage coinImage = getItemImage(1, 0);
            BufferedImage fullImage = new BufferedImage(natureRuneImage.getWidth() + alchingItemImage.getWidth() + coinImage.getWidth(), alchingItemImage.getHeight(), BufferedImage.TYPE_3BYTE_BGR);
            Graphics g = fullImage.getGraphics();
            g.drawImage(natureRuneImage, 0, 0, null);
            g.drawImage(alchingItemImage, natureRuneImage.getWidth(), 0, null);
